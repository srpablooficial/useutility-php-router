...
class Router
{
    ...
    public function create_controller($name, $folder_path)
    {
        try {
            ...
            if (!$name || !$folder_path) {
                throw new Exception("verificar path o nombre de la ruta", 1);
            }
    ...
    public function run(string $routeURL = '', string $contentType = null)
    {
        try {
            ...
            foreach ($this->routes as $path => $values) {
                ...
                if (isset($settings) && is_array($settings)) {
                    ...
                    if (isset($settings["src"])) {
                        ...
                        if (preg_match("/(.*)\/(.*)/i", $settings["src"], $settings_path_matched)) {
                            ...
                            $name_class = ucfirst(str_replace("@controller", '', strtolower($settings["src"])));
                        }
                    }
                ...
                if (preg_match_all("/\/(\w+)\/{(\w+)}/i", $path, $path_match) && preg_match_all("/\/(\w+)\/([\w%]+)/i", $requestPath, $requestpath_match) && !isset($settings["param"])) {
            ...
            $this->noFound["message"] = $th->getMessage();
